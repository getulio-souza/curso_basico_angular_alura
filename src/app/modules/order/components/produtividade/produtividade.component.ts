...
import { CollumnDefinition, GridOrderData } from '../../../dynamic-widgets/collapse-grid/collapse-grid.component';
...
export class ProdutividadePageComponent extends PropertyDataLoader implements OnInit {
  loading = false;
  ...
  // GRID
  gridData: any[] = [{}];
  graficoGrid: 'Pedidos por Situação x Tempo de Espera' | 'Pedidos finalizados fora do SLA por Área' | 'Pedidos por Área' | 'Pedidos por Ala' = 'Pedidos por Situação x Tempo de Espera';
  ...
  async ngOnInit() {
    ...
    this.dateFilterSubject.pipe(debounceTime(500))
    .subscribe((_) => {
      this.dispatchFilter(null);
    });

    this.dateRangeFilterSubject.pipe(debounceTime(500))
    .subscribe((_) => {
      this.dispatchDateRange();
    });

    this.advancedFilterSubject.pipe(debounceTime(5000))
    .subscribe((orders) => {
  ...
  async initView(view: string): Promise<void> {
    if (view === 'admin') {
      ...
      setTimeout(() => {
        ...
        this.findEventsCategoriesOpennedAtHalfhour(null, null, null, null, null);
        this.topFiveCategoriesWithOpennedOrders(null, null, null, null, null);
        this.topFiveWardsAndOppenedOrdersQuantityForTable(null, null, null, null, null);
      }, 1500);
    } 
    if (view === 'detail') {
      ...
      const rangeString = [new Date(), new Date()].map(date => date.getTime()).toString().replace("[", "").replace("]", "");
      this.orderService.ordersFilteredForAdvancedView(null , null, null, null, null, rangeString, null, null)
      .subscribe(resp => {
  ...
  setRangeDate(event): void {
    this.rangeSelected = [event.startDate, event.endDate].map(date => new Date(date));
  ...
  dispatchFilter(category: string): void {
    ...
    this.topFiveCategoriesWithDelayedOrders(null, null, null, initial.getTime(), final.getTime());
  }
  ...
  halfChartClickEvent(event: {to: string, value: any, extras: any}): void {
    if (event.to === 'category' || event.to === null) {
      ...
      this.findEventsCategoriesOpennedAtHalfhour(event.extras.ward, null, event.extras.subject, null, null);
    }
    if (event.to === 'item') {
      this.findEventsOpennedAtHalfhour(event.extras.ward, event.value, event.extras.subject, null, null);
    }
  }

  private findEventsCategoriesOpennedAtHalfhour(ward, category, subject, start, end): void {
    this.orderEventService.findEventsCategoriesOpennedAtHalfhour(ward, category, subject, start, end)
    .subscribe((response: OrderCategoryQuantityDTO[]) => {
      this.halfHourData = response.map(category => {
        this.loadingSubject.next();
        return Object.assign({}, {label: category.category, value: [category.quantity, category.min, category.max]});
      });
    });
  }

  private findEventsOpennedAtHalfhour(ward, category, subject, start, end): void {
    this.orderEventService.findEventsOpennedAtHalfhour(ward, category, subject, start, end)
      .subscribe((events: OrderEvent[]) => {
        this.halfHourData = [];
        if (events.length) {
          const itens = new Set(events.map(item => item.item));
          itens.forEach(item => {
            const label = events.find(obj => obj.item === item).labels ? events.find(obj => obj.item === item).labels['pt'] : null;
            const quantity = events.filter(obj => obj.item === item).map(obj => parseInt(obj.quantity)).reduce((act, nxt) => act + nxt);
            const min = events.filter(obj => obj.item === item).map(obj => obj.timestamp).reduce((prv, crt) => {
              return (prv > crt) ? crt : prv;
            });
            const max = events.filter(obj => obj.item === item).map(obj => obj.timestamp).reduce((prv, crt) => {
              return !(prv > crt) ? crt : prv;
            });
            this.halfHourData.push(Object.assign({}, {label, value: [quantity, max, min]}));
          });
        }
    });
  }
  ...
  clickPizza(event: {to: string, value: any}): void {
    if (event.to === 'category') {
      ...
      this.topFiveWardsAndOppenedOrdersQuantity(null, event.value, null, null, null);
    } else if (event.to === 'item') {
      this.findEventsOpennedOrders(event.value, this.cachePizzaCategory, null, null, null);
    }
  }

  private topFiveCategoriesWithOpennedOrders(ward, category, subject, start, end): void {
    this.orderEventService.topFiveCategoriesWithOpennedOrders(ward, category, subject, start, end)
    .subscribe((response: OrderCategoryQuantityDTO[]) => {
      this.topFiveOppened = response.map(category => {
        this.loadingSubject.next();
        return Object.assign({}, {label: category.category, value: [category.quantity]});
      });
    });
  }

  private topFiveWardsAndOppenedOrdersQuantity(ward, category, subject, start, end): void {
    this.orderEventService.topFiveWardsAndOppenedOrdersQuantity(ward, category, subject, start, end)
    .subscribe((response: OrderWardQuantityDTO[]) => {
      this.topFiveOppened = response.map(ward => {
        return Object.assign({}, {label: ward.ward, value: [ward.quantity]});
      });
    });
  }

  private findEventsOpennedOrders(ward, category, subject, start, end): void {
    this.orderEventService.findEventsOpennedOrders(ward, category, subject, start, end)
    .subscribe((events: OrderEvent[]) => {
      this.topFiveOppened = [];
      if (events.length) {
        const itens = new Set(events.map(item => item.item));
        itens.forEach(item => {
          const label = events.find(obj => obj.item === item).labels ? events.find(obj => obj.item === item).labels['pt'] : null;
          const quantity = events.filter(obj => obj.item === item).map(obj => parseInt(obj.quantity)).reduce((act, nxt) => act + nxt);
          this.topFiveOppened.push(Object.assign({}, {label, value: [quantity]}));
        });
      }
    });
  }
  ...
  private topFiveCategoriesWithDelayedOrders(ward, category, subject, start, end): void {
    this.orderEventService.topFiveCategoriesWithDelayedOrders(ward, category, subject, start, end)
    .subscribe((response: OrderCategoryQuantityDTO[]) => {
      this.loadingSubject.next();
  ...
  private findEventsDelayed(ward, category, subject, start, end): void {
    this.orderEventService.findEventsDelayed(ward, category, subject, start, end)
    .subscribe((events: OrderEvent[]) => {
      this.afterSlaData = [];
      if (events.length) {
        const itens = new Set(events.map(item => item.item));
        itens.forEach(item => {
          const label = events.find(obj => obj.item === item).labels ? events.find(obj => obj.item === item).labels['pt'] : null;
          const quantity = events.filter(obj => obj.item === item).map(obj => parseInt(obj.quantity)).reduce((act, nxt) => act + nxt);
          this.afterSlaData.push(Object.assign({}, {label, value: [quantity]}));
        });
      }
    });
  }

  private topFiveWardsAndOppenedOrdersQuantityForTable(ward, category, subject, start, end): void {
    this.orderEventService.topFiveWardsAndOppenedOrdersQuantity(ward, category, subject, start, end)
    .subscribe((response: OrderWardQuantityDTO[]) => {
      this.data = response.map(ward => {
        this.loadingSubject.next();
        return Object.assign({}, {ward: ward.ward, quantity: ward.quantity});
      });
    });
  }
  ...
  selectedFilterEvent(event): void {
    ...
    this.findEventsCategoriesOpennedAtHalfhour(event.ward, null, event.subject, null, null);
  }
  ...
  selecionarStatusDetail(event: string): void {
    ...
    this.orderService.ordersFilteredForAdvancedView(status, this.categoriesAdvancedSelecionado, this.sectorsAdvancedSelecionado, this.wardsAdvancedSelecionado, this.subjectsAdvancedSelecionado, datas, this.slaConvertOption(), null)
    .subscribe(resp => {
      this.advancedFilterSubject.next(resp);
    })
  }
  ...
  selecionarCategoriaDetail(event: string): void {
    ...
    const datas = this.rangeSelected.map(date => date.getTime()).toString().replace("[", "").replace("]", "");
    this.orderService.ordersFilteredForAdvancedView(status, event, this.sectorsAdvancedSelecionado, this.wardsAdvancedSelecionado, this.subjectsAdvancedSelecionado, datas, this.slaConvertOption(), null)
    .subscribe(resp => {
      this.advancedFilterSubject.next(resp);
    })
  }
  ...
  selecionarSetorDetail(event: string): void {
    ...
    const status = this.statusAdvancedSelecionado ? this.orderService.orderStatus().get(this.statusAdvancedSelecionado) : null;
    const datas = this.rangeSelected.map(date => date.getTime()).toString().replace("[", "").replace("]", "");
    this.orderService.ordersFilteredForAdvancedView(status, this.categoriesAdvancedSelecionado, event, this.wardsAdvancedSelecionado, this.subjectsAdvancedSelecionado, datas, this.slaConvertOption(), null)
    .subscribe(resp => {
      this.advancedFilterSubject.next(resp);
    })
  }
  ...
  selecionarAlaDetail(event: string): void {
    ...
    this.orderService.ordersFilteredForAdvancedView(status, this.categoriesAdvancedSelecionado, this.sectorsAdvancedSelecionado, event, null, datas, this.slaConvertOption(), null)
    .subscribe(resp => {
      this.advancedFilterSubject.next(resp);
    })
  }
  ...
  selecionarUnidadeDetail(event: string): void {
    ...
    const status = this.statusAdvancedSelecionado ? this.orderService.orderStatus().get(this.statusAdvancedSelecionado) : null;
    const datas = this.rangeSelected.map(date => date.getTime()).toString().replace("[", "").replace("]", "");
    this.orderService.ordersFilteredForAdvancedView(status, this.categoriesAdvancedSelecionado, this.sectorsAdvancedSelecionado, this.wardsAdvancedSelecionado, event, datas, this.slaConvertOption(), null)
    .subscribe(resp => {
      this.advancedFilterSubject.next(resp);
    })
  }
  ...
  slaConvertOption(): string {
    if (this.slaAdvancedSelecionado === null || this.slaAdvancedSelecionado === 'Todos') {
      return 'none';
  ...
  async advancedOrganize(orders) {
    ...
    const periods = [0.5, 1, 1.5, 2, 2.5, 3, 3.5, 4, 4.5, 5, 5.5, 6, 6.5, 7, 7.5, 8, 8.5, 9, 9.5, 10];

    const itens = new Set(orders.map(order => order.orderItems[0].item.labels['pt']));
    const itemQuantity = [];
    await itens.forEach(item => {
      const quantity = (orders.filter(order => order.orderItems[0].item.labels['pt'] === item)
    ...
    const itemPeriod = [];
    await itens.forEach(item => {
      itemPeriod.push(Object.assign({}, {item, periods: new Map<string, number>()}));
      periods.forEach(period => {
        itemPeriod.find(obj => obj.item === item).periods.set(`${period}`, 0);
      });
      itemPeriod.find(obj => obj.item === item).periods.set('+10', 0);

      const mapped = orders.filter(order => (order.orderItems[0].item.labels['pt'] === item));
      mapped.forEach(order => {
        const minuteTotal = ((actualTime - new Date(order.createdAt).getTime()) / 1000 / 60);
        const horas = (minuteTotal / 60).toFixed(2);
        const split = horas.split(".");
        const formated = parseFloat(`${split[0]}.${((Math.round(parseInt(split[1].substring(0, 2)))) < 50 ? 5 : 0)}`);
        const quantity = order.orderItems[0].quantity;

        if (periods.includes(formated)) {
          itemPeriod.find(obj => obj.item === item).periods.set(`${formated}`, (itemPeriod.find(obj => obj.item === item).periods.get(`${formated}`) + quantity));
        } else {
          itemPeriod.find(obj => obj.item === item).periods.set('+10', (itemPeriod.find(obj => obj.item === item).periods.get('+10') + quantity));
        }
      });
    });

    const categories = new Set(orders.map(order => order.orderItems[0].item.categoryId));
    const categoriesQuantity = [];
    await categories.forEach(category => {
      const quantity = (orders.filter(order => order.orderItems[0].item.categoryId === category)
        .map(order => order.orderItems[0].quantity)
        .reduce((acc, pvr) => acc + pvr)
      );
    ...
    await categories.forEach(category => {
      categoriesPeriod.push(Object.assign({}, {category, periods: new Map<string, number>()}));
      periods.forEach(period => {
        categoriesPeriod.find(obj => obj.category === category).periods.set(`${period}`, 0);
      });
      categoriesPeriod.find(obj => obj.category === category).periods.set('+10', 0);

      const mapped = orders.filter(order => order.orderItems[0].item.categoryId === category);
      mapped.forEach(order => {
        const minuteTotal = ((actualTime - new Date(order.createdAt).getTime()) / 1000 / 60);
        const horas = (minuteTotal / 60).toFixed(2);
        const split = horas.split(".");
        ...
        if (periods.includes(formated)) {
          categoriesPeriod.find(obj => obj.category === category).periods.set(`${formated}`, (categoriesPeriod.find(obj => obj.category === category).periods.get(`${formated}`) + quantity));
        } else {
          categoriesPeriod.find(obj => obj.category === category).periods.set('+10', (categoriesPeriod.find(obj => obj.category === category).periods.get('+10') + quantity));
        }
      });
    });
    ...
    await wards.forEach(ward => {
      const quantity = (orders.filter(order => order.wardFullQualified === ward)
        .map(order => order.orderItems[0].quantity)
        .reduce((acc, pvr) => acc + pvr)
      );
    ...
    await categories.forEach(async category => {
      const mapped = (
        orders.filter(order => (order.orderItems[0].item.categoryId === category) 
          && (order.deliveryEstimate !== null && order.finishedAt !== null) 
          && (new Date(order.deliveryEstimate).getTime() < new Date(order.finishedAt).getTime())
        )
      );
      const quantity = await mapped.map(order => order.orderItems[0].quantity).reduce((acc, pvr) => acc + pvr, 0);
    ...
    if (this.graficoGrid === 'Pedidos finalizados fora do SLA por Área') {
      const mapped = (
        orders.filter(order => (order.deliveryEstimate !== null && order.finishedAt !== null) 
          && (new Date(order.deliveryEstimate).getTime() < new Date(order.finishedAt).getTime())
        )
      );
  ...
  convertOrderToGridData(order): GridOrderData {
    const doneTime = (doneStamp, createdStamp) => {
      return `${((new Date(doneStamp).getTime() - new Date(createdStamp).getTime()) / 1000 / 60).toFixed(0)} min`;
    }
    ...
    return Object.assign({}, {
      ...
      created: order.createdAt ? new Date(order.createdAt).toLocaleString('pt-BR') : null,
      updated: order.lastUpdate ? new Date(order.lastUpdate).toLocaleString('pt-BR') : null,
      finished: order.finishedAt ? new Date(order.finishedAt).toLocaleString('pt-BR') : null,
      estimated: order.deliveryEstimate ? new Date(order.deliveryEstimate).toLocaleString('pt-BR') : null,
      doneTime: order.finishedAt ? doneTime(order.finishedAt, order.createdAt) : null,
  ...
  refresh(): void {
    this.loading = false;
    if (this.adminView) {
      this.findEventsCategoriesOpennedAtHalfhour(this.cacheWard, null, this.cacheSubject, null, null);
      this.topFiveCategoriesWithOpennedOrders(null, null, null, null, null);
      this.topFiveWardsAndOppenedOrdersQuantityForTable(null, null, null, null, null);
      this.dispatchFilter(null);
    } else {
