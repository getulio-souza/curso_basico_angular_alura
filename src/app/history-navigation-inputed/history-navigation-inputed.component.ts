...
export class HistoryNavigationInputedComponent implements OnInit, AfterViewInit {
  ...
  periods = ['últimas 24h', 'últimas 48h', 'última semana', 'último mês'];
  ...
  pickPeriod(period: 'últimas 24h' | 'últimas 48h' | 'última semana' | 'último mês') {
    this.period = period;
  ...
  filterByDate(): void {
    this.onViewChange.emit({ startDate: this.rangeDates[0], endDate: this.rangeDates[1], resolution: this.resolution });
  }
}
