...
import { CollumnDefinition, GridNPSData } from '../collapse-grid-nps/collapse-grid-nps.component';
...
export class NPSPageComponent extends PropertyDataLoader implements OnInit {
  ...
  constructor(translateService: TranslateService, structureService: StructureService, propertiesService: PropertiesService, private dateService: DateService, private npsEventService: NPSEventService) {
  ...
  ngOnInit() {
    ...
    this.dateFilterSubject.pipe(debounceTime(1500))
    .subscribe((_) => {
      this.dispatchFilter();
    });
  ...
  async initView(view: string): Promise<void> {
    ...
    if (view === 'detail') {
      ...
      this.npsEventService.getCategoriesAndSubjects()
  ...
  selectedFilterEvent(event, chartIndex): void {
    ...
    switch (chartIndex) {
      case CHART_INDEX.NPS_BY_CATEGORY:
        this.findNPSByCategory(this.startDate, this.endDate, category, null, null, null, true);
        this.selectedCategoryNpsByCategoryFilter = category;
        break;
      case CHART_INDEX.NPS_CLOSED_ORDERS:
        this.findNPSClosedOrders(this.startDate, this.endDate, category, null, null, null, true);
  ...
  dispatchFilter(): void {
    ...
    this.findEventsGlobalSatisfaction(initial.getTime(), final.getTime(), null, null, null, null);
    this.findNPSClosedOrders(initial.getTime(), final.getTime(), null, null, null, null);
    this.findCategoriesNPSLowerThanSeven(initial.getTime(), final.getTime(), null, null, null, null);
    this.findNPSByCategory(initial.getTime(), final.getTime(), null, null, null, null);
  }
  ...
  dispatchFilterDetail(): void {
    ...
    this.findEventsGlobalSatisfaction(initial.getTime(), final.getTime(), this.categoriesAdvancedSelecionado, this.sectorAdvancedSelecionado, this.wardsAdvancedSelecionado, this.subjectsAdvancedSelecionado);
    this.findNPSClosedOrders(initial.getTime(), final.getTime(), this.categoriesAdvancedSelecionado, this.sectorAdvancedSelecionado, this.wardsAdvancedSelecionado, this.subjectsAdvancedSelecionado);
    this.findCategoriesNPSLowerThanSeven(initial.getTime(), final.getTime(), this.categoriesAdvancedSelecionado, this.sectorAdvancedSelecionado, this.wardsAdvancedSelecionado, this.subjectsAdvancedSelecionado);
    this.findNPSByCategory(initial.getTime(), final.getTime(), this.categoriesAdvancedSelecionado, this.sectorAdvancedSelecionado, this.wardsAdvancedSelecionado, this.subjectsAdvancedSelecionado);
  }
  ...
  private findEventsGlobalSatisfaction(start, end, category, sector, ward, subject): void {
    this.npsEventService.findEventsGlobalSatisfaction(start, end, category, sector, ward, subject)
    .subscribe((response: NPSCategoryGlobalSatisfactionDTO) => {
      ...
      if (response && response.dataChart) {
        ...
        this.globalSatisfactionData = response.dataChart.map(data => {
          return Object.assign({}, {label: data.category, value: [data.average]});
        });
      }
    });
  }

  private findNPSByCategory(start, end, category, sector, ward, subject, fromFilter = false): void {
    this.npsEventService.findNPSByCategory(start, end, category, sector, ward, subject)
    .subscribe((response: NPSCategoryDTO) => {
      ...
      if (response && response.dataChart) {
        ...
        if (!fromFilter) {
          this.seletorCategoryNPSByCategoryDataChart = [EMPTY_OPTION, ...response.categoriesFilter];
        }
        this.npsByCategoryData = response.dataChart.map(item => {
          return Object.assign({}, {label: item.dateRef, value: [item.average]});
        });
      }
    });
  }

  private findNPSClosedOrders(start, end, category, sector, ward, subject, fromFilter = false): void {
    this.npsEventService.findNPSClosedOrders(start, end, category, sector, ward, subject)
    .subscribe((response: NPSClosedOrderDTO) => {
      ...
      if (response && response.dataChart) {
        this.chartByCategoryDataDetail = response.details;
        if (!fromFilter) {
          this.seletorCategoryClosedOrdersDataChart = [EMPTY_OPTION, ...response.categoriesFilter];
        }
  ...
  private findCategoriesNPSLowerThanSeven(start, end, category, sector, ward, subject): void {
    this.npsEventService.findCategoriesNPSLowerThanSeven(start, end, category, sector, ward, subject)
    .subscribe((response: NPSCategoryLowerThanSevenDTO) => {
      if (response && response.dataChart) {
        this.chartLowerThanSevenDataDetail = response.details;
        this.categoriesNpsLowerThanSevenData = response.dataChart.map(data => {
          return Object.assign({}, {label: data.category, value: [data.average]});
        });
      }
    });
  ...
  convertNPSToGridData(nps): GridNPSData {
    ...
    return Object.assign({}, {
      ...
      created: nps.createdAt ? new Date(nps.createdAt).toLocaleString('pt-BR') : null,
      updated: nps.lastUpdate ? new Date(nps.lastUpdate).toLocaleString('pt-BR') : null,
      finished: nps.finishedAt ? new Date(nps.finishedAt).toLocaleString('pt-BR') : null,
      estimated: nps.deliveryEstimate ? new Date(nps.deliveryEstimate).toLocaleString('pt-BR') : null,
      doneTime: nps.finishedAt ? doneTime(nps.finishedAt, nps.createdAt) : null,
