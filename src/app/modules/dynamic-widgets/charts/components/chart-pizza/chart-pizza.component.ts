...
export class ChartPizzaComponent implements OnInit {
  ...
  @ViewChild('chartPizzaCanvas', { static: true }) chartPizzaCanvas: ElementRef<HTMLDivElement>;
  ...
  async configureChart(itens: SingleDataStatistics[]): Promise<void> {
    ...
    const series = itens.map(item => Object.assign({}, {name: item.label, value: item.value[0], label: {color: '#FFFFFF'}}));
