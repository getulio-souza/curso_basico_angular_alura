...
{
  @Output() clickBarVerticalEvent: EventEmitter<{
    to: string;
    value: any;
    extras: any;
  }> = new EventEmitter<{ to: string; value: any; extras: any }>();
  ...
  @ViewChild(ChartBarVerticalComponent, { static: false })
  chartBarVertical: ChartBarVerticalComponent;
  @ViewChild(ChartBarHorizontalComponent, { static: false })
  chartBarHorizontal: ChartBarHorizontalComponent;
  @ViewChild(ChartPizzaComponent, { static: false })
  chartPizza: ChartPizzaComponent;
  @ViewChild(ChartLinesComponent, { static: false })
  chartLines: ChartLinesComponent;
  @ViewChild(ChartLinesNpsComponent, { static: false })
  chartLinesNps: ChartLinesNpsComponent;
  @ViewChild(ChartFunnelComponent, { static: false })
  chartFunnel: ChartFunnelComponent;
  @ViewChild(ChartGaugeComponent, { static: false })
  chartGauge: ChartGaugeComponent;
  @ViewChild(ChartStackedLineComponent, { static: false })
  chartStackedLine: ChartStackedLineComponent;
  @ViewChild(ChartBarVerticalSimpleComponent, { static: false })
  chartBarVerticalSimple: ChartBarVerticalSimpleComponent;
  @ViewChild(ChartBarVerticalNpsComponent, { static: false })
  chartBarVerticalNps: ChartBarVerticalNpsComponent;
  ...
  ngOnChanges(changes: SimpleChanges): void {
    ...
    if (
      changes.seletorCategory &&
      changes.seletorCategory.currentValue.length
    ) {
    ...
    if (
      changes.seletorSubjects &&
      changes.seletorSubjects.currentValue.length
    ) {
  ...
  charts: any[] = [
    {
      type: CHART_TYPES.BAR_HORIZONTAL,
      attribute: "chartBarHorizontal",
      shouldApply: true,
    },
    {
      type: CHART_TYPES.BAR_VERTICAL,
      attribute: "chartBarVertical",
      shouldApply: true,
    },
    ...
    {
      type: CHART_TYPES.STACKED_LINE,
      attribute: "chartStackedLine",
      shouldApply: true,
    },
    {
      type: CHART_TYPES.BAR_VERTICAL_SIMPLE,
      attribute: "chartBarVerticalSimple",
      shouldApply: true,
    },
    {
      type: CHART_TYPES.BAR_VERTICAL_NPS,
      attribute: "chartBarVerticalNps",
      shouldApply: true,
    },
    {
      type: CHART_TYPES.LINES_NPS,
      attribute: "chartLinesNps",
      shouldApply: true,
    },
  ];
  ...
  clickVertical(event: { to: string; value: any }): void {
    this.clickBarVerticalEvent.emit({
      ...event,
      extras: { ward: this.selectedWard, subject: this.selectedSubject },
    });
  }
