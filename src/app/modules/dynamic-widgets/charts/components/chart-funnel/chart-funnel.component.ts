...
export class ChartFunnelComponent implements OnInit {
  @ViewChild('chartFunnelCanvas', { static: true }) chartFunnelCanvas: ElementRef<HTMLDivElement>;
  ...
  configureChart(itens: SingleDataStatistics[]) {
    ...
    const totalData = itens.map(item => item.value[0]).reduce((prv, crt) => prv + crt);
    const data = itens.map(item => Object.assign({}, { name: item.label, value: this.getPerccentage(totalData, item.value[0])} ));
