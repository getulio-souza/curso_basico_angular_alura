...
@Component({
  selector: 'home-property-summary2',
  templateUrl: './property-summary2.component.html',
  styleUrls: ['./property-summary2.component.less']
})
export class PropertySummaryComponent2 extends PropertyDataLoader implements OnInit, OnDestroy, OnChanges {
  ...
  @Output() cardSummaryClickEmitter = new EventEmitter();
  ...
  constructor(
    ...
    //first time should has
    // the same behaviour as after back
    this.afterBack = true;
  ...
  ngOnChanges(changes: SimpleChanges): void {
    ...
    // if floorIdFromFloorplan has changed, lets find which floor was select
    //  and act like card summary for the given floor was clicked
    if(changes.floorIdFromFloorplan != null && !changes.floorIdFromFloorplan.firstChange){
      this.findFloorClick();
    }
  }
  ...
  findFloorClick(){
    this.subdivisionSummaries.every(element => {
      const floorId = element.id;
      if(this.floorIdFromFloorplan == floorId){
        this.onCardSummaryClick(element);
        return false;
      }
      return true;
    });
  ...
  onChangeStructureFilter(filteredStructure){
    ...
    if(!this.subdivisionLevel) {
      // depending on order to loading components
      // subdivisionLevel still be null
      // so lets update it
      this.updateSubdivisionLevel();
    }
  ...
  buildSubdivisionSummaries(subdivisionIds) {
    ...
    subdivisionIds.forEach(subdivisionId => {
      updates.push(this.updateSubdivisionData(subdivisionId, true));
    });

    if(this.afterBack){
      this.propertySummaryState.emitIsLoadingSummaries(true);
    }
    forkJoin(updates).subscribe((res) => {
      this.propertySummaryState.emitIsLoadingSummaries(false);
  ...
  updateSubdivisionData(subdivisionId, forceRequest) {
    ...
    return this.buildStructureSummary(summary['structure'], summary, forceRequest).pipe(map(summary => {
  ...
  onDotSubdivisionClick(summary) {
    ...
    this.cardSummaryClickEmitter.emit(summary);
  }
  ...
  startGettingSubdivisionData(summary) {
    ...
    this.realTime.startGettingRealTimeData(() => {
      ...
      this.updateSubdivisionData(summary.structureId, true).subscribe((res) => {});
    })
  ...
  backToPropertySummary() {
    ...
    //emit null since no floor has been selected
    this.cardSummaryClickEmitter.emit(null);
  }
