...
export class RatingCardComponent2 implements OnInit, OnDestroy {
    ...
    switch (indicatorId) {
      case "sold":
        return this.calculateIndicator(
          this.indicatorService.getSoldIndicator(this.propertyId)
        );
      case "presence":
        return this.calculateIndicator(
          this.indicatorService.getPresenceIndicator(this.propertyId)
        );
      case "savings":
        return this.calculateIndicator(
          this.indicatorService.getSavingsIndicator()
        );
      case "staffPresence":
        return this.calculateIndicator(
          this.indicatorService.getStaffPresence()
        );
      case "alerts":
        return this.calculateIndicator(
          this.indicatorService.getAlertIndicator()
        );
      case "waterLevel":
        return this.calculateIndicator(
          this.indicatorService.getWaterLevelIndicator()
        );
      default:
  ...
  calculateIndicator(indicatorServiceFunc: Observable<any>) {
    return indicatorServiceFunc.pipe(
      map((res) => {
        if (this.total == null) {
          this.total = 0;
        }
